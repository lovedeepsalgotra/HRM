...
<div class="header bg-primary pb-6">
    ...
    <form action="{{route('updatepackage')}}" method="POST" class="form" enctype="multipart/form-data">
      @csrf
    ...
    <div class="form-group">
      <br>
        ...
        <input class="form-control" type="text" value="{{$getpacdata->title}}"name="title" required>
    </div>
    <div class="form-group">
        <label for="example-text-input" class="form-control-label">Packages Price*</label>
        <input class="form-control" type="text" value="{{$getpacdata->price}}"name="price" required>
    </div>
...
<div class="form-group">
    ...
    @if($packagescategorylists->id==$assign_category->category_id)
    <option selected value="{{$packagescategorylists->id}}">{{$packagescategorylists->category_name}}</option>
@else
...
<div class="module">
    ...
    ?>
      ...
      @foreach($getsubdata as $key=> $subtitle1)
...
<div class="form-group" id="showsubtitle">
    ...
    <input class="form-control" value="{{$subtitle1->subtitle}}" name="package[subtitle<?php echo $key+1; ?>][]" required>
...
@if($subtitle1->id==$test->package_subtitle_id)
<?php
...
<div class="form-group"id="show_item">
    ...
    <input class="form-control" value="{{$test->test_name}}" id="testname" name="package[subtitle<?php echo $key+1; ?>][test_name<?php echo $key+1; ?>][]" >
...
<input type="hidden" id="count_value"name="subtitle_count" value="<?php echo $count; ?>">
    </div>
