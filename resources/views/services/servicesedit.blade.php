...
<div class="header bg-primary pb-6">
    ...
    <form action="{{route('updateservices')}}" method="post" class="form" enctype="multipart/form-data">
      @csrf

      <div class="custom-file">
      <label for="example-text-input" class="form-control-label">Current Image</label><br>
      <img alt="" style="width:10%" src="{{ asset('ServicesImage') }}/{{{$getservicedata->image}}}">
       ...
       <div class="custom-file">
        <br> <br>
       <label for="example-text-input" class="form-control-label">Add New Image for Service </label><br>
    ...
    <br>
        ...
        <input class="form-control" type="text" value="{{$getservicedata->title}}"name="title" required>
    </div>
    <!--<div class="form-group">-->
    <!--    <label for="example-text-input" class="form-control-label">Tagline</label>-->
    <!--    <input class="form-control" type="text" value="{{$getservicedata->tagline}}"name="tagline">-->
    <!--</div>-->
  ...
  @if($icon->id==$getservicedata->icon)
  ...
  @endforeach
    ...
    <div class="form-group">
        ...
        <input class="form-control" type="link" value="{{$getservicedata->iconlink}}"name="iconlink">
    </div>
  
  
    <!--<div class="form-group">-->
    <!--    <label for="example-email-input" class="form-control-label">Heading</label>-->
    <!--    <input class="form-control" type="email" value="{{$getservicedata->heading}}" name="heading">-->
    <!--</div>-->
    <label class="Short-Description">Service Short-Description for home page</label>
    <textarea name="short_des" style="width:100%" value="">{{$getservicedata->short_des}}
</textarea>
   
    <label for="Description" class="form-control-label">Add Content For Single Page (like description,Image etc).</label>
    <textarea id="basic-example" name="description" value="">{{$getservicedata->description}}
</textarea>
